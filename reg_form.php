<?php
 ...
 if(!empty($_POST))
 {
	...
	if($empty)
	{	
		header("location: reg_form.php?info=Vyplňte prosím všechny údaje !!!");
		$uspech=false;
	}
	  ...
	  while ($Kontrola_loginu=mysql_fetch_row($jmena))
	  {
	  	foreach ($Kontrola_loginu as $login)
		{
			...
			{
				header("location: reg_form.php?info=tento login nebo E-mail již existuje !!!");
				$uspech=false;
			}
		}
	  }
		...
		{
			header("location: reg_form.php?info=hesla se neshodují !!!");
			$uspech=false;
		}
		...
		{
			header("location: reg_form.php?info=Heslo je příliš krátké, musí obsahovat alespoň 8 znaků !!!");
			$uspech=false;
		}
		...
		{	
			...
			if($vlozeni)
			{
				...
				header("location: reg_form.php?info=Registrace proběhla úspěšně & correct=true");
				exit();
			}
            ...
            <input name="submit_reg" type="submit" value=" Registrovat ">
            	 ...
            	 <?php 
            	 	...
            	 	{	
            	 		...
            	 		echo "<input type=\"button\" value=\"Zavřít\" onclick=\"window.close()\">";
            	  	}
